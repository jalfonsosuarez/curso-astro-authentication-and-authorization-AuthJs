  ...
  input: z.object({
    ...
    imageFiles: z
      .array(
        z
          .instanceof(File)
          .refine((file) => file.size <= MAX_FILE_SIZE, "Max file image 5Mb")
          .refine((file) => {
            if (file.size === 0) return true;
            return ACEPTED_IMAGE_FILE.includes(file.type);
          }, `Only supported image files are valid ${ACEPTED_IMAGE_FILE.join(", ")}`)
      )
      .optional(),
  }),
